...
int		check_shadow(t_ray *shadow_ray, t_light *light, t_object *object,
	t_prog *prog)
{
	t_object	*tmp;
	double		dist;
...
void	fill_diffuse_color(t_light_p *lp, t_light *light,
	t_vec3 **cumu_r_vcolor, t_vec3 **previous_vcolor)
{
	t_vec3	*lambertian;
	t_vec3	*light_vcolor;
	t_vec3	*r_vcolor;
...
t_vec3	*get_diffuse_shading(t_light_p *lp, t_prog *prog)
{
	...
	while (light)
	{
		shadow_ray = new_ray(vec3_cpy(lp->hit_p),
			vec3_get_nvec3_between(light->pos, lp->hit_p));
...
t_vec3	*get_shading_point(t_ray *ray, t_object *object, t_prog *prog)
{
	...
	previous_vcolor = NULL;
	ambient_vcolor = NULL;
	if (!(lp = get_light_p_object(ray, object)))
